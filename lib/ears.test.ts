	...
	it("rejects ubiquitous requirement without THE SYSTEM SHALL", () => {
		const result = validateEarsGrammar(
			req(
				"REQ-001",
				"ubiquitous",
				"The system should log all operations.",
			),
		);
		assert.ok(
			result !== null,
			"Expected an error for missing THE SYSTEM SHALL",
		);
	...
	it("accepts a valid event-driven requirement", () => {
		const result = validateEarsGrammar(
			req(
				"REQ-002",
				"event-driven",
				"WHEN a user logs in, THE SYSTEM SHALL validate the token.",
			),
		);
		assert.equal(result, null);
	});
	...
	it("rejects event-driven without THE SYSTEM SHALL response", () => {
		const result = validateEarsGrammar(
			req(
				"REQ-002",
				"event-driven",
				"WHEN a user logs in, the system should validate",
			),
		);
		assert.ok(result !== null);
		assert.ok(result!.includes("THE SYSTEM SHALL"));
	...
	it("accepts a valid state-driven requirement", () => {
		const result = validateEarsGrammar(
			req(
				"REQ-003",
				"state-driven",
				"WHILE the migration runs, THE SYSTEM SHALL queue writes.",
			),
		);
		assert.equal(result, null);
	});
	...
	it("accepts a valid optional requirement", () => {
		const result = validateEarsGrammar(
			req(
				"REQ-004",
				"optional",
				"WHERE dark mode is enabled, THE SYSTEM SHALL render dark UI.",
			),
		);
		assert.equal(result, null);
	});
	...
	it("accepts a valid complex requirement with WITHIN", () => {
		const result = validateEarsGrammar(
			req(
				"REQ-005",
				"complex",
				"WITHIN 500ms of query, THE SYSTEM SHALL return results.",
			),
		);
		assert.equal(result, null);
	});
	...
	it("accepts a valid complex requirement with WHEN ... AND", () => {
		const result = validateEarsGrammar(
			req(
				"REQ-005",
				"complex",
				"WHEN import completes AND report has zero errors, THE SYSTEM SHALL promote data.",
			),
		);
		assert.equal(result, null);
	});
...
describe("detectConflict", () => {
	it("detects allow/deny conflict on same object", () => {
		const a = req(
			"R1",
			"ubiquitous",
			"THE SYSTEM SHALL allow guest access to public data.",
		);
		const b = req(
			"R2",
			"ubiquitous",
			"THE SYSTEM SHALL deny guest access to public data.",
		);
	...
	it("detects create/delete conflict on same object", () => {
		const a = req(
			"R1",
			"ubiquitous",
			"THE SYSTEM SHALL create audit records on login.",
		);
		const b = req(
			"R2",
			"ubiquitous",
			"THE SYSTEM SHALL delete audit records on login.",
		);
		const result = detectConflict(a, b);
	...
	it("detects enable/disable conflict", () => {
		const a = req(
			"R1",
			"ubiquitous",
			"THE SYSTEM SHALL enable notifications for all users.",
		);
		const b = req(
			"R2",
			"ubiquitous",
			"THE SYSTEM SHALL disable notifications for all users.",
		);
		const result = detectConflict(a, b);
	...
	it("detects same trigger with different responses", () => {
		const a = req(
			"R1",
			"event-driven",
			"WHEN a user submits form, THE SYSTEM SHALL process the data.",
		);
		const b = req(
			"R2",
			"event-driven",
			"WHEN a user submits form, THE SYSTEM SHALL reject the data.",
		);
		const result = detectConflict(a, b);
	...
	it("returns null for opposite verbs on different objects", () => {
		const a = req(
			"R1",
			"ubiquitous",
			"THE SYSTEM SHALL allow admin access to settings.",
		);
		const b = req(
			"R2",
			"ubiquitous",
			"THE SYSTEM SHALL deny guest access to reports.",
		);
		const result = detectConflict(a, b);
	...
	it("detects 'appropriate' as ambiguous", () => {
		const result = detectAmbiguity(
			req(
				"R1",
				"ubiquitous",
				"THE SYSTEM SHALL take appropriate action.",
			),
		);
	...
	it("detects 'timely' as ambiguous", () => {
		const result = detectAmbiguity(
			req(
				"R1",
				"ubiquitous",
				"THE SYSTEM SHALL respond in a timely manner.",
			),
		);
		assert.ok(result.length > 0);
	...
	it("detects multiple ambiguous terms", () => {
		const result = detectAmbiguity(
			req(
				"R1",
				"ubiquitous",
				"THE SYSTEM SHALL efficiently process various data properly, etc.",
			),
		);
		assert.ok(
			result.length >= 3,
			`Expected at least 3 ambiguities, got ${result.length}`,
		);
	});
	...
	it("returns empty array for clear requirement", () => {
		const result = detectAmbiguity(
			req(
				"R1",
				"ubiquitous",
				"THE SYSTEM SHALL hash passwords using bcrypt with cost 12.",
			),
		);
...
describe("analyzeRequirements", () => {
	it("returns grammar issues for invalid EARS requirements", () => {
		...
		assert.ok(
			...
			"Expected grammar errors to be categorized as incompleteness",
		);
	});
	...
	it("detects ambiguous language", () => {
		const issues = analyzeRequirements([
			req(
				"R1",
				"ubiquitous",
				"THE SYSTEM SHALL handle requests in a timely manner.",
			),
		]);
	...
	it("reports incompleteness when error handling is missing", () => {
		...
		const incompleteness = issues.filter(
			(i) => i.type === "incompleteness",
		);
		// Should flag missing error, invalid, unauthorized
		assert.ok(
			incompleteness.length >= 2,
			`Expected incompleteness issues, got ${incompleteness.length}`,
		);
	});
	...
	it("returns no grammar issues for fully valid requirements", () => {
		const issues = analyzeRequirements([
			...
			req(
				"R2",
				"event-driven",
				"WHEN an error occurs, THE SYSTEM SHALL send an alert.",
			),
		]);
		...
		assert.equal(
			...
			0,
			"No grammar errors for valid requirements",
		);
	});
});
...
describe("formatRequirement", () => {
	it("uppercases THE SYSTEM SHALL in ubiquitous requirements", () => {
		...
		assert.ok(
			result.includes("THE SYSTEM SHALL"),
			`Expected uppercase, got: ${result}`,
		);
	});
	...
	it("uppercases WHEN in event-driven requirements", () => {
		...
		assert.ok(
			result.includes("WHEN "),
			`Expected uppercase WHEN, got: ${result}`,
		);
		assert.ok(
			result.includes("THE SYSTEM SHALL"),
			`Expected THE SYSTEM SHALL after trigger, got: ${result}`,
		);
	});
	...
	it("uppercases WHILE in state-driven requirements", () => {
		...
		assert.ok(
			...
			`Expected uppercase WHILE, got: ${result}`,
		);
	});
	...
	it("uppercases WHERE in optional requirements", () => {
		...
		assert.ok(
			result.includes("WHERE "),
			`Expected uppercase WHERE, got: ${result}`,
		);
	});
	...
	it("includes pattern name in formatted output", () => {
		const result = formatRequirement(
			req(
				"REQ-001",
				"event-driven",
				"WHEN a user logs in, THE SYSTEM SHALL validate token.",
			),
		);
		assert.ok(
			result.includes("[Event-Driven]"),
			`Expected pattern name, got: ${result}`,
		);
	});
	...
	it("each pattern has name, description, template, and examples", () => {
		for (const [key, val] of Object.entries(EARS_PATTERNS)) {
			...
			assert.ok(
				...
				`Pattern ${key} missing description`,
			);
			assert.ok(
				val.template.length > 0,
				`Pattern ${key} missing template`,
			);
			assert.ok(
				val.examples.length > 0,
				`Pattern ${key} missing examples`,
			);
		}
	});
});

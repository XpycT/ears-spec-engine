...
import { EARS_PATTERNS, analyzeRequirements, formatAnalysisReport, validateEarsGrammar } from "./lib/ears.js";
import { renderClarifyingQuestions, analyzeDependencies } from "./lib/templates.js";
...
export default function earsSpecEngine(pi: ExtensionAPI): void {
  ...
  function specDir(featureName: string): string {
    const sanitized = featureName
      ...
      .replace(/^-|-$/g, "");
    if (!sanitized) {
      throw new Error(`Invalid feature name: "${featureName}" produces an empty directory name after sanitization.`);
    }
  ...
  function updateStatusWidget(ctx: ExtensionContext): void {
    if (state.requirements) {
      const totalReqs = state.requirements.stories.reduce((s, st) => s + st.requirements.length, 0);
      const parts: string[] = [];
      parts.push(`📋 ${state.requirements.stories.length} stories, ${totalReqs} EARS reqs`);
      if (state.lastAnalysis && state.lastAnalysis.length > 0) {
        const high = state.lastAnalysis.filter((a) => a.severity === "high").length;
        const medium = state.lastAnalysis.filter((a) => a.severity === "medium").length;
        parts.push(ctx.ui.theme.fg(high > 0 ? "error" : medium > 0 ? "warning" : "muted",
          `${high + medium} issue(s)`));
      }
  ...
  pi.registerTool({
    name: "ears_validate",
    label: "EARS Validate",
    description: "Validate one or more EARS requirements against the 5 canonical patterns (Ubiquitous, Event-Driven, State-Driven, Optional, Complex). Returns grammar errors and pattern usage report.",
    ...
    parameters: Type.Object({
      requirements: Type.Array(Type.Object({
        ...
        pattern: Type.Enum({ ubiquitous: "ubiquitous", "event-driven": "event-driven", "state-driven": "state-driven", optional: "optional", complex: "complex" }),
        statement: Type.String({ description: "The full EARS requirement statement" }),
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
      ...
      const report = invalid.length === 0
        ...
        : `❌ ${invalid.length}/${results.length} requirements have grammar errors:\n${
          invalid.map((r) => `  - ${r.id}: ${r.error}`).join("\n")
        }\n\nPattern distribution: ${patternSummary}`;
  ...
  pi.registerTool({
    name: "ears_analyze",
    label: "EARS Analyze",
    description: "Analyze EARS requirements for logical conflicts, ambiguous language, incompleteness (missing edge cases), and redundancy. Produces a severity-ranked report.",
    ...
    parameters: Type.Object({
      requirements: Type.Array(Type.Object({
        ...
        storyId: Type.String({ description: "Story ID this requirement belongs to" }),
      })),
    }),

    async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
      const typedReqs = params.requirements.map((r) => ({ ...r, pattern: r.pattern as EarsPattern }));
  ...
  pi.registerTool({
    ...
    description: "Analyze a list of spec tasks, build a dependency graph, and return optimal execution waves (parallel-friendly groups) and the critical path. Each wave can run independently; tasks within a wave that are marked as parallel can run concurrently.",
    ...
    parameters: Type.Object({
      tasks: Type.Array(Type.Object({
        ...
        dependencies: Type.Array(Type.String(), { description: "IDs of tasks this task depends on" }),
        parallel: Type.Boolean({ description: "Whether this task can run in parallel with siblings" }),
      })),
    }),
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
      ...
      for (let w = 0; w < analysis.waves.length; w++) {
        ...
        lines.push(`### Wave ${w}${w === 0 ? " (Foundation)" : w === analysis.waves.length - 1 ? " (Final)" : ""}`);
      ...
      return {
        ...
        details: { waves: analysis.waves.length, criticalPath: analysis.criticalPath },
      };
    },
  });
  ...
  pi.registerCommand("ears:quick-plan", {
    description: "Quick Plan mode — clarify requirements, then generate requirements, design, and tasks in one pass (Kiro Quick Plan equivalent)",
    handler: async (args, ctx) => {
      if (!args || args.trim().length === 0) {
        ctx.ui.notify("Usage: /ears:quick-plan <feature description>", "warning");
        return;
      }
...
${renderClarifyingQuestions(args.trim()).map((q, i) => `${i + 1}. ${q}`).join("\n")}
...
Specs will be saved to: ${featureDir}/`,
  ...
  pi.registerCommand("ears:spec", {
    description: "Phase 1: Generate EARS requirements document from a feature description",
    handler: async (args, ctx) => {
      if (!args || args.trim().length === 0) {
...
${Object.values(EARS_PATTERNS).map((val) => `- **${val.name}**: ${val.template}`).join("\n")}
...
- No ambiguous terms (appropriate, timely, efficient, etc.)`,
  ...
  pi.registerCommand("ears:analyze", {
    description: "Analyze existing requirements for conflicts, ambiguities, and completeness. Reads from the spec directory if no state is loaded.",
    handler: async (_args, ctx) => {
      ...
      if (!state.requirements && state.currentSpecsDir) {
        try {
          ...
          ctx.ui.notify(`Found requirements.md at ${reqPath}. Use the \`ears_analyze\` tool with the parsed requirements to get a full analysis.`, "info");
          return;
        } catch {
          ctx.ui.notify("No requirements loaded and no requirements.md found. Create them first with /ears:spec", "warning");
          return;
        }
      }

      if (!state.requirements) {
        ctx.ui.notify("No requirements loaded. Create them first with /ears:spec", "warning");
        return;
      }
      ...
      ctx.ui.notify(`Found ${analysis.length} issue(s) — ${analysis.filter((a) => a.severity === "high").length} high`, analysis.length > 0 ? "warning" : "info");
    },
  });
  ...
  pi.registerCommand("ears:design", {
    description: "Phase 2: Generate a design document from the approved requirements",
    handler: async (_args, ctx) => {
      if (!state.currentSpecsDir) {
        ctx.ui.notify("No specs directory. Start with /ears:spec first", "warning");
        return;
      }
...
Present for user approval after generation.`,
  ...
  pi.registerCommand("ears:tasks", {
    description: "Phase 3: Generate task breakdown with dependency analysis from requirements + design",
    handler: async (_args, ctx) => {
      if (!state.currentSpecsDir) {
        ctx.ui.notify("No specs directory. Complete /ears:spec first", "warning");
        return;
      }
...
Present for user approval after generation.`,
  ...
  pi.registerCommand("ears:status", {
    description: "Show current EARS spec engine state — loaded requirements, design, tasks",
    handler: async (_args, ctx) => {
      const parts: string[] = [];
      parts.push("📋 **EARS Spec Engine Status**\n");

      if (state.currentSpecsDir) {
        parts.push(`- Specs directory: \`${state.currentSpecsDir}\``);
      }

      if (state.requirements) {
        const totalReqs = state.requirements.stories.reduce((s, st) => s + st.requirements.length, 0);
        parts.push(`- ✅ Requirements: ${state.requirements.stories.length} stories, ${totalReqs} EARS requirements`);
      } else {
      ...
      if (state.lastAnalysis && state.lastAnalysis.length > 0) {
        const high = state.lastAnalysis.filter((a) => a.severity === "high").length;
        const med = state.lastAnalysis.filter((a) => a.severity === "medium").length;
        parts.push(`- 🔍 Analysis: ${state.lastAnalysis.length} issue(s) (${high} high, ${med} medium)`);
      }
  ...
  pi.on("session_start", async (_event, ctx) => {
    for (const entry of ctx.sessionManager.getEntries()) {
      if (entry.type === "custom" && (entry as { customType?: string }).customType === "ears-spec-state") {
